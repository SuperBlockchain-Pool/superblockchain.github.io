var api = "https://superblockchain.con-ip.com:8135";
let parentCoin = "Zent"

var email = "david@securitypc.es";
var telegram = "https://t.me/joinchat/FGO99xKu_KNJCwCJcu3kow";
var discord = "https://discord.gg/ANvE54E";
var facebook = "https://www.facebook.com/superblockchainpool";
var twitter = "https://twitter.com/SuperBlockchaim";
var instagram = "https://www.instagram.com/superblockchain/";

var marketCurrencies = ["{symbol}-BTC", "{symbol}-XMR", "{symbol}-LTC", "{symbol}-DOGE", ];

var blockchainExplorer = "http://explorer.zent.cash/?hash={id}#blockchain_block";
var transactionExplorer = "http://explorer.zent.cash/?hash={id}#blockchain_transaction";

var themeCss = "themes/default.css";
var defaultLang = 'en';
